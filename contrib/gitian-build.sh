...
if [[ $setup = true ]]
then
  sudo echo "Running setup."
  sudo apt-get -y install ruby apache2 git apt-cacher-ng python-vm-builder qemu-kvm qemu-utils binutils
  ...
  then
    ...
    bin/make-base-vm --suite --arch amd64
  fi
  popd
...
#!/bin/sh -e
brctl addbr br0
ip addr add ${BR0_PREFIX}.1/24 broadcast ${BR0_PREFIX}.255 dev br0
ip link set br0 up
firewall-cmd --zone=trusted --add-interface=br0
exit 0
EOF"

  # either make a back or restore one
  if [ ! -f /home/$(whoami)/.bashrc_orig ]
  then
    sudo cp /home/$(whoami)/.bashrc /home/$(whoami)/.bashrc_orig
  else 
    sudo cp /home/$(whoami)/.bashrc_orig /home/$(whoami)/.bashrc
  fi
  sudo bash -c "cat << EOF >> /home/$(whoami)/.bashrc
export USE_LXC=1
export GITIAN_HOST_IP=${BR0_PREFIX}.1
export LXC_GUEST_IP=${BR0_PREFIX}.5
export GITIAN_SIGNER=${SIGNER}
EOF"

  echo ""
  echo "You must restart your gitian host by running 'sudo init 6'"
  echo ""
  exit 0
fi
...
then
  ...
  then
    echo ""
    echo "Compiling v${VERSION} Linux"
    echo ""
  ...
  then
    echo ""
    echo "Compiling v${VERSION} Windows"
    echo ""
  ...
  then
    echo ""
    echo "Compiling v${VERSION} Mac OSX"
    echo ""
